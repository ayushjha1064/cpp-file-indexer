...
\begin{tikzpicture}[
  ...
  store/.style={draw, rounded corners, fill=green!10, minimum width=2.4cm,
                minimum height=0.8cm, align=center},
  arrow/.style={-{Stealth[length=2.2mm]}, thick}
]
  \node[component] (input) {Large log file};
  \node[component, below=of input] (reader) {POSIX read()\\or mmap()};
  \node[component, below=of reader] (producer) {Delimiter-aligned\\producer};
  \node[component, below=of producer] (queue) {Bounded blocking\\queue};
  \node[component, below left=1.1cm and 1.4cm of queue] (workers)
        {Worker threads\\tokenize + index};
  \node[store, below right=1.1cm and 1.4cm of queue] (merge)
        {Serial merge\\WordIndex};
  \node[store, below=of merge] (persistent) {Persistent version\\optional};
  \node[component, below=of persistent] (query) {Query engine\\word / Top-K / diff};
